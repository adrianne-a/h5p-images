...
H5P.MultiChoice = function (options, contentId, contentData) {
  ...
  // Keep track of number of correct choices追踪正确选择的数量
  var numCorrect = 0;

  console.log(params);


  // Loop through choices 循环选择
  for (var i = 0; i < params.answers.length; i++) {
    var answer = params.answers[i];

    // Make sure tips and feedback exists确保存在提示和反馈
    answer.tipsAndFeedback = answer.tipsAndFeedback || {};

    if (params.answers[i].correct) {
      // Update number of correct choices更新正确选择的数量
      numCorrect++;
    }
  }

  // Determine if no choices is the correct判断是否没有选择是正确的
  var blankIsCorrect = (numCorrect === 0);

  // Determine task type确定任务类型
  if (params.behaviour.type === 'auto') {
    // Use single choice if only one choice is correct如果只有一个选项是正确的，则使用单选
  ...
  // Initialize buttons and elements.初始化按钮和元素。
  var $myDom;
  var $feedbackDialog;

  /**
   * Remove all feedback dialogs移除所有反馈对话框
   */
  var removeFeedbackDialog = function () {
    // Remove the open feedback dialogs.移除打开的反馈对话框。
  ...
  /**
   * Add feedback to element向元素添加反馈
  ...
  /**
   * Register the different parts of the task with the H5P.Question structure. 用H5P.Question结构注册任务的不同部分。
   */
  self.registerDomElements = function () {
    ...
    for (let i = 0; i < params.answers.length; i++) {
      ...
      const $divContainer = $('<div>', {
        class: "h5p-alternative-container",
        html: '<span class="h5p-alternative-inner">' + answer.text + '</span>',
        appendTo: $list
      })

      if (answer.media.type) {
        const { path, alt, title } = answer.media.type.params.file;
        $('<div>', {
          class: 'option-Images',
    ...
    var toggleCheck = function ($ans) {
      ...
      if (params.behaviour.singleAnswer) {
        ...
        if ($ans.attr('aria-checked') === 'true') {
          ...
          // Do not allow un-checking when retry disabled and auto check禁止重试和自动检查时不允许取消检查
    ...
    // Adds check and retry button增加了检查和重试按钮
    addButtons();
    if (!params.behaviour.singleAnswer) {
  ...
  var addQuestionToXAPI = function (xAPIEvent) {
    ...
    definition.description = {
      ...
      //删除标签，必须用div标签包裹，因为如果字符串没有用标签包裹，jQuery 1.9会崩溃。
      ...
      if (params.answers[i].correct) {
        if (!params.singleAnswer) {
          if (definition.correctResponsesPattern.length) {
            ...
            // This looks insane, but it's how you separate multiple answers这看起来很疯狂，但这是你如何分开多个答案的方法
